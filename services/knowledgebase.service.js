...
function buildMarkdown(insightsList) {
    ...
    for (const ci of insightsList) {
        ...
        if (ci.avgRecentBalance != null) md += `Average Recent Balance: ${rupees(ci.avgRecentBalance)}  \n`;
        if (ci.creditUtilizationPct != null && ci.creditUtilizationPct >= 0) md += `Credit Card Utilization: ${ci.creditUtilizationPct}%  \n`;
        if (ci.subscriptionBurden > 0) md += `Monthly Subscription Burden: ${rupees(ci.subscriptionBurden)}  \n`;
        md += '\n';

        if (ci.topCategories?.length > 0) {
            md += '### Top Spending Categories\n\n';
            ci.topCategories.forEach((c) => { md += `- ${c.category}: ${rupees(c.amount)}\n`; });
            md += '\n';
        }

        if (ci.subscriptions?.length > 0) {
            md += '### Active Subscriptions\n\n';
            ci.subscriptions.forEach((s) => { md += `- ${s}\n`; });
            md += '\n';
        }

        if (ci.insights?.length > 0) {
            md += '### Key Observations\n\n';
            ci.insights.forEach((obs) => { md += `- ${obs}.\n`; });
            md += '\n';
        } else {
            md += '### Key Observations\n\n';
...
async function ingestViaUrl(token) {
    try {
        ...
        if (!PUBLIC_API_URL) throw createError(500, 'PUBLIC_API_URL environment variable is not configured');
        ...
        console.error('Error ingesting knowledgebase to Bolna:', error.response?.data || error.message || error);
    }
}
...
exports.generateKnowledgebase = async (payload) => {
    try {
        ...
        if (!insightsList.length) throw createError(500, 'Failed to generate insights for any customer');
    ...
    try {
        ...
        const creditCardDetails = creditCards.map((card) => {
            ...
            const dueDate = card.metadata?.paymentDueDate
                ? new Date(card.metadata.paymentDueDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })
                : null;
            const billingEnd = card.metadata?.billingCycleEnd
                ? new Date(card.metadata.billingCycleEnd).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })
                : null;
            ...
            return {
                ...
                utilizationStatus: utilizationPct >= 90 ? 'CRITICAL' : utilizationPct >= 70 ? 'HIGH' : utilizationPct >= 40 ? 'MODERATE' : 'HEALTHY',
        ...
        const loanDetails = loans.map((loan) => {
            ...
            const nextEmiDate = loan.metadata?.nextEmiDate
                ? new Date(loan.metadata.nextEmiDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })
                : null;
            const remainingMonths = loan.metadata?.remainingTenureMonths || null;
            const loanEndDate = loan.metadata?.loanEndDate
                ? new Date(loan.metadata.loanEndDate).toLocaleDateString('en-IN', { month: 'short', year: 'numeric' })
                : null;
            ...
            return {
                ...
                foreclosureAmountFormatted: loan.metadata?.foreclosureAmount ? rupees(loan.metadata.foreclosureAmount) : null,
        ...
        const fdDetails = fds.map((fd) => ({
            ...
            maturityDate: fd.metadata?.maturityDate
                ? new Date(fd.metadata.maturityDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })
                : null,
        ...
        const rdDetails = rds.map((rd) => ({
            ...
            maturityDate: rd.metadata?.maturityDate
                ? new Date(rd.metadata.maturityDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })
                : null,
        ...
        const totalOutstanding = loans.reduce((s, l) => s + (l.metadata?.totalOutstandingAmount || 0), 0);

        const alerts = [];
        creditCardDetails.forEach((c) => {
            if (c.utilizationPercent >= 90) alerts.push(`${c.productName} ending ${c.last4Digits} is at ${c.utilizationPercent}% utilization — critically high`);
            if (c.paymentDueDate) alerts.push(`${c.productName} payment due on ${c.paymentDueDate}`);
        });
        loanDetails.forEach((l) => {
            if (l.latePayments > 0) alerts.push(`${l.productName} had ${l.latePayments} late EMI payment(s) in the selected period`);
            if (l.nextEmiDate) alerts.push(`${l.productName} next EMI of ${l.emiAmountFormatted} due on ${l.nextEmiDate}`);
        });
        fdDetails.forEach((fd) => {
            ...
            if (daysToMaturity !== null && daysToMaturity <= 30 && daysToMaturity >= 0) {
                alerts.push(`FD "${fd.productName}" matures on ${fd.maturityDate} — auto-renewal is ${fd.autoRenewal ? 'ON' : 'OFF'}`);
            }
        });
...
exports.getCustomerContext = async (payload) => {
    try {
        ...
        const recentTxns = transactions.slice(0, 5).map((t) => ({
            date: new Date(t.transactionDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' }),
        ...
        const lastBalance = transactions.find((t) => t.balanceAfterTransaction != null || t.balance != null);
        ...
        if (latestRisk?.riskLevel === 'HIGH' || latestRisk?.riskLevel === 'CRITICAL') {
            alerts.push(`Customer is flagged as ${latestRisk.riskLevel} risk (score: ${latestRisk.riskScore}/100)`);
        }
        ...
        const profile = {
            ...
            onboardingDate: customer.onboardingDate
                ? new Date(customer.onboardingDate).toLocaleDateString('en-IN', { month: 'short', year: 'numeric' })
                : null,
        };
