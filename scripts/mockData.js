...
const neftBeneficiaries = [
  'Rajesh Sharma – HDFC Bank',
  'Priya Patel – ICICI Bank',
  'Amit Kumar – SBI',
  'Mutual Fund SIP – Axis AMC',
  'Insurance Premium – LIC',
  'Society Maintenance – HSG Soc',
  'Tuition Fee – ABC School',
  'Medical Payment – Max Healthcare',
];

const impsBeneficiaries = [
  'Savings Transfer – Self Account',
  'Family Support – Parent Account',
  'Rent Payment – Landlord',
  'Freelance Payment Received',
  'EMI Pre-payment Transfer',
];

const atmBranches = [
  'Mumbai Andheri West Branch',
  'Delhi Connaught Place Branch',
  'Bangalore Koramangala Branch',
  'Hyderabad Banjara Hills Branch',
  'Chennai T Nagar Branch',
  'Pune Hinjewadi Branch',
  'Mumbai Powai Branch',
  'Delhi Saket Branch',
  'Bangalore Whitefield Branch',
];
...
const standingInstructionPayees = [
  ...
  'Broadband Internet – Airtel',
  'Mobile Postpaid – Jio',
  'DTH Recharge – Tata Play',
  'Housing Society Maintenance',
  'SIP – HDFC Mutual Fund',
  'SIP – ICICI Prudential MF',
];

const merchantMCCs = {
  'Swiggy Online Services': '5812',
  'Zomato Food Delivery': '5812',
  'Amazon Pay Merchant': '5411',
  'PhonePe Merchant': '6012',
  'Google Pay Transfer': '6012',
  'Paytm Wallet Load': '6012',
  'BigBasket Groceries': '5411',
  'Dunzo Delivery': '5411',
  'Uber India': '4121',
  'Ola Cabs': '4121',
  'Flipkart Marketplace': '5311',
  'DMart Ready': '5411',
  'Apollo Pharmacy': '5912',
  BookMyShow: '7832',
  MakeMyTrip: '4722',
  'Zepto Quick Commerce': '5411',
  'Blinkit Groceries': '5411',
  'Cred Bill Pay': '6012',
  'Amazon India': '5311',
  'Flipkart Online': '5311',
  'Reliance Digital': '5732',
  'Croma Electronics': '5732',
  'Shoppers Stop': '5311',
  'Lifestyle International': '5311',
  'Myntra Fashion': '5651',
  'Decathlon Sports': '5941',
  'IKEA India': '5712',
  'Nykaa Beauty': '5977',
  'Tata CLiQ': '5311',
  'HP Petrol Pump': '5541',
  'Indian Oil Station': '5541',
  'BPCL Fuel Station': '5541',
  'PVR Cinemas': '7832',
  'Starbucks India': '5814',
  'Haldirams Restaurant': '5812',
  'Taj Hotels': '7011',
  'Marriott Hotels': '7011',
  'MakeMyTrip Flights': '4511',
  'Netflix Subscription': '4899',
  'Amazon Prime Membership': '5968',
  'Spotify Premium': '4899',
  'YouTube Premium': '4899',
  'Hotstar VIP': '4899',
};
...
async function generateCustomers(count = 20) {
  ...
  return saved.map((c, idx) => ({
    ...c.toObject(),
    _behaviorProfile: customers[idx]._behaviorProfile,
  }));
}
...
async function generateProducts(customer) {
  ...
  const emiAmount = Math.round(
    (loanAmount * (loanRate / 1200) * Math.pow(1 + loanRate / 1200, loanTenure)) /
      (Math.pow(1 + loanRate / 1200, loanTenure) - 1),
  );
...
function generateSavingsTransactions(product, customer, profile) {
  ...
  switch (profile) {
    case 'FINANCIALLY_HEALTHY':
      balance = Math.round(randomFloat(4, 8) * monthlyIncome);
      break;
    case 'MEDIUM_RISK':
      balance = Math.round(randomFloat(2, 4) * monthlyIncome);
      break;
    case 'HIGH_RISK':
      balance = Math.round(randomFloat(0.5, 1.5) * monthlyIncome);
      break;
    case 'NEAR_DEFAULT':
      balance = Math.round(randomFloat(0.15, 0.5) * monthlyIncome);
      break;
    default:
      balance = Math.round(randomFloat(1.5, 3) * monthlyIncome);
  }
  ...
  for (let month = 0; month < 12; month++) {
    ...
    const rentAmount =
      profile === 'FINANCIALLY_HEALTHY' ? random(15000, 25000) : random(10000, 20000);
    balance -= rentAmount;
    txns.push({
      transactionId: txnId(),
      customerId,
      ...
      if (Math.random() < 0.4) {
        ...
        txns.push({
          ...
          transactionDate: new Date(
            year,
            m,
            random(20, Math.min(28, daysInMonth)),
            14,
            random(0, 59),
          ),
    ...
    enrichTxn(t, {
      ...
      channel:
        t.instrument === 'UPI'
          ...
          : t.instrument === 'ATM'
            ? 'ATM'
            : t.instrument === 'NEFT' || t.instrument === 'IMPS'
              ? pick(['INTERNET_BANKING', 'MOBILE_APP'])
              : pick(channels),
      merchantName: (t.description || '').includes('–')
        ? (t.description || '').split('–').pop().trim().split(' ').slice(0, 3).join(' ')
        : null,
    }),
  );
}
  ...
  return txns.map((t) =>
    enrichTxn(t, {
      ...
      channel:
        t.instrument === 'CARD'
          ? 'POS'
          : t.instrument === 'BANK_INTERNAL_TRANSFER'
            ? 'INTERNET_BANKING'
            : pick(channels),
      merchantName: (t.description || '').includes('–')
        ? (t.description || '').split('–').pop().trim().split(' ').slice(0, 3).join(' ')
        : null,
      subCategory:
        t.transactionCategory === 'FOOD'
          ? pick(['Dining', 'Delivery', 'Cafe'])
          : t.transactionCategory === 'SHOPPING'
            ? pick(['Electronics', 'Fashion', 'Groceries'])
            : null,
      tags:
        t.transactionCategory === 'FOOD'
          ? ['food']
...
async function runSeeder() {
  try {
    ...
    console.log(
      `Summary: ${result.customers} customers, ${result.products} products, ${result.transactions} transactions`,
    );
