...
exports.calculateRisk = async (payload) => {
  try {
    ...
    const recentSavingsTxns = transactions
      .filter(
        (t) => t.productType === 'SAVINGS_ACCOUNT' && t.balance !== null && t.balance !== undefined,
      )
    ...
    const SUBSCRIPTION_KEYWORDS =
      /netflix|spotify|aws|gym|saas|youtube|prime|hotstar|disney|swiggy\s?one|zomato\s?pro/i;
    if (monthlyIncome > 0) {
    ...
    if (spendingValues.length >= 2) {
      ...
      const variance =
        spendingValues.reduce((sum, v) => sum + Math.pow(v - meanSpending, 2), 0) /
    ...
    let riskScore =
      savingsScore + spendingSpikeScore + balanceHealthScore + subscriptionScore + volatilityScore;
    ...
    if (savingsScore > 0) {
      const savingsRate =
        totalCredits > 0 ? (((totalCredits - totalDebits) / totalCredits) * 100).toFixed(1) : 0;
      insights.push(
        `Savings rate dropped below ${savingsScore >= 30 ? '10%' : '20%'} (currently ${savingsRate}%)`,
      );
    }
    if (spendingSpikeScore > 0) {
      insights.push(
        `Spending increased by ${Math.round(spendingIncreasePercent)}% compared to last month`,
      );
    }
    if (balanceHealthScore > 0 && lastBalance !== null) {
      insights.push(
        `Average balance fell below threshold (₹${lastBalance.toLocaleString('en-IN')})`,
      );
    }
...
exports.getRiskSummary = async (payload) => {
  try {
    ...
    const latestRisk = await RiskScores.findOne({
      customerId: new mongoose.Types.ObjectId(customerId),
    })
      .sort({ year: -1, month: -1 })
      .lean();
    ...
    if (latestRisk?.breakdown) {
      ...
      if (savingsScore > 0)
        topRiskFactors.push({ factor: 'Low Savings Rate', score: savingsScore });
      if (spendingSpikeScore > 0)
        topRiskFactors.push({ factor: 'Spending Spike', score: spendingSpikeScore });
      if (balanceHealthScore > 0)
        topRiskFactors.push({ factor: 'Low Balance', score: balanceHealthScore });
      if (subscriptionScore > 0)
        topRiskFactors.push({ factor: 'High Subscription Burden', score: subscriptionScore });
      if (volatilityScore > 0)
        topRiskFactors.push({ factor: 'High Volatility', score: volatilityScore });
