...
const SUBSCRIPTION_KEYWORDS = /netflix|spotify|youtube\s?premium|prime|hotstar|disney|adobe|microsoft\s?365|linkedin|cult\.fit|apple\s?icloud|gym\s?membership/i;
const KNOWN_SUBSCRIPTION_SERVICES = [
    'Netflix', 'Spotify', 'Amazon Prime', 'YouTube Premium', 'Hotstar', 'Disney+',
    'Adobe Creative Cloud', 'Microsoft 365', 'Apple iCloud', 'LinkedIn Premium', 'Cult.fit',
];
...
exports.generateCustomerInsights = async (customerId) => {
    try {
        ...
        const subscriptionLabels = [
            ...new Set(subscriptionTxns.map(resolveSubscriptionLabel)),
        ...
        const totalAvailableLimit = creditCards.reduce((s, p) => s + (p.metadata?.availableLimit || 0), 0);
        ...
        const avgRecentBalance = balances.length > 0
        ...
        if (spendingSpikePercent > 30) {
            insights.push(
                `Spending increased by ${spendingSpikePercent}% compared to last month`,
            );
        }
        if (creditUtilizationPct > 80) {
            insights.push(
                `Credit card utilization exceeds ${creditUtilizationPct}% of credit limit`,
            );
        }
        if (lowBalanceEvents > 3) {
            insights.push('Balance drops below ₹5,000 near month end');
        }
        if (monthlySubscriptionBurden > 0 && monthlyIncome > 0 && monthlySubscriptionBurden > monthlyIncome * 0.1) {
